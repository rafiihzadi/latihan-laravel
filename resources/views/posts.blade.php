...
<div class="card mb-3">
   ...
   <img src="{{ asset('storage/' . $posts[0]->image) }}" alt="{{ $posts[0]
   ...
   <p>
    ...
    class="text-decoration-none">{{ $posts[0]->author->name}}</a> in <a href="/posts?
    category={{ $posts[0]->category->slug }}" class="text-decoration-none">{{ $posts[0]
   ->category->name }}</a> {{ $posts[0]->create_at->diffForHumans() }}
    </small>
   </p>
...
<div class="container">
   <div class="row">
    ...
    <p>
    <small class="text-muted">  
    By.<a href="/posts?autho={{ $post->author->username }}" 
